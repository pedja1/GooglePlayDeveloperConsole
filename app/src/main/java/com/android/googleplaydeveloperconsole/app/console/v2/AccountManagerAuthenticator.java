...
public class AccountManagerAuthenticator extends BaseAuthenticator
{
    ...
    {
        super(accountName);
        this.accountManager = AccountManager.get(MainApp.getContext());
    ...
    @Override
    public SessionCredentials authenticate(Activity activity, boolean invalidate) throws AuthenticationException
    {
    ...
    private SessionCredentials authenticateInternal(Activity activity, boolean invalidate) throws AuthenticationException
    {
        try
        {
            ...
            if (account == null)
            {
                throw new AuthenticationException(String.format("Account %s not found on device?", accountName));
            }
            ...
            Bundle authResult = accountManager.getAuthToken(account, "weblogin:service=androiddeveloper&continue=" + DEVELOPER_CONSOLE_URL, false,
                    null, null).getResult();
            if (authResult.containsKey(AccountManager.KEY_INTENT))
            {
                ...
                if (activity == null)
                {
                    ...
                    builder.setContentText(ctx.getResources().getString(R.string.auth_error, accountName));
                    builder.setAutoCancel(true);
                    PendingIntent contentIntent = PendingIntent.getActivity(ctx, accountName.hashCode(), authIntent, PendingIntent.FLAG_UPDATE_CURRENT);
